...
def build_node_config(node_id: str, flow_data: Dict[str, Any]) -> Optional[NodeConfig]:
    ...
    if "context_strategy" in data:
        ...
        if isinstance(strategy_config, str):
            ...
            strategy = getattr(
                ContextStrategy,
                strategy_config.get("strategy", "APPEND").upper(),
                ContextStrategy.APPEND,
            )
            context_strategy = ContextStrategyConfig(
                strategy=strategy, summary_prompt=strategy_config.get("summary_prompt")
            )
...
def load_flow_from_json(json_data: Dict[str, Any]) -> NodeConfig:
    ...
    if not initial_node:
        ...
        if nodes:
            ...
            logger.warning(
                f"No node of type 'initial' found. Using the first node: {initial_node.get('id')}"
            )
        else:
