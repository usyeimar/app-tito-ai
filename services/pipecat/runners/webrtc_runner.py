...
def cli() -> None:
    ...
    parser = argparse.ArgumentParser(description="Unified WebRTC Bot Runner")
    ...
    if loaded_assistant:
        # tools: use capabilities (v3) if present, else fall back to agent.tools (v1)
        active_tools = (
            [t.model_dump() for t in loaded_assistant.capabilities.tools if not t.disabled]
            if loaded_assistant.capabilities.tools
            else loaded_assistant.agent.tools
        )
        config.tools = active_tools
        config.flow_config = loaded_assistant.flow
        # pipeline_settings.inactivity_messages already returns the correct dict format
        config.inactivity_messages = loaded_assistant.pipeline_settings.inactivity_messages
        ...
        config.initial_message_interruptible = (
            loaded_assistant.pipeline_settings.initial_message_interruptible
        )
    ...
    async def main():
        ...
        if not room_url or not token:
            print("\n" + "=" * 50)
            print("🔄 CREATING NEW DAILY SESSION...")
        ...
        if loaded_assistant and loaded_assistant.webhooks:
            ...
            print(
                f"🔗 Webhook configuration loaded: {webhook_config.url} (Events: {webhook_config.events})"
            )
        else:
